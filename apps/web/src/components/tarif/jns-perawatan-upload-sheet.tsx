...
export const JnsPerawatanUploadSheet = () => {
  const [open, setOpen] = useState(false);
  const [file, setFile] = useState<File | null>(null);
  const [parsedData, setParsedData] = useState<JnsPerawatanData[]>([]);
  ...
  const insertTarifMutation = useMutation({
    ...trpc.tarif.insertTarif.mutationOptions(),
  });
  ...
  const handleInsertData = async () => {
    ...
    try {
      ...
      for (let i = 0; i < parsedData.length; i++) {
        ...
        try {
          await insertTarifMutation.mutateAsync(data);
          successCount++;
        } catch (err) {
    ...
    <Sheet open={open} onOpenChange={setOpen}>
      <SheetTrigger asChild>
        <Button>
          ...
          Upload Jenis Perawatan CSV
        </Button>
      </SheetTrigger>
      <SheetContent className="w-[600px] sm:w-[700px]">
        <SheetHeader>
          <SheetTitle>Upload Jenis Perawatan CSV</SheetTitle>
          <SheetDescription>
            Upload a CSV file with jenis perawatan data to import into the
            ...
            menejemen, total_byrdr, total_byrpr, total_byrdrpr, kd_pj, kd_poli,
            status
          </SheetDescription>
        </SheetHeader>
