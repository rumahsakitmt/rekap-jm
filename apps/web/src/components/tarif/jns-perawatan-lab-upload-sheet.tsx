...
export const JnsPerawatanLabUploadSheet = () => {
  const [open, setOpen] = useState(false);
  const [file, setFile] = useState<File | null>(null);
  const [parsedData, setParsedData] = useState<JnsPerawatanLabData[]>([]);
  ...
  const insertTarifLabMutation = useMutation({
    ...trpc.tarif.insertTarifLab.mutationOptions(),
  });
  ...
  const handleInsertData = async () => {
    ...
    try {
      ...
      for (let i = 0; i < parsedData.length; i++) {
        ...
        try {
          await insertTarifLabMutation.mutateAsync(data);
          successCount++;
        } catch (err) {
      ...
      if (errorCount === 0) {
        ...
        queryClient.invalidateQueries({
          queryKey: trpc.tarif.getTarifLab.queryKey(),
        });
        handleUploadSuccess();
      } else {
        setError(
  ...
  return (
    <Sheet open={open} onOpenChange={setOpen}>
      ...
      <SheetContent className="w-[600px] sm:w-[700px]">
        <SheetHeader>
          <SheetTitle>Upload Lab CSV</SheetTitle>
          <SheetDescription>
            Upload a CSV file with lab data to import into the system. The CSV
            should contain columns: kdJenisPrw, nmPerawatan, bagianRs, bhp,
            tarifPerujuk, tarifTindakanDokter, tarifTindakanPetugas, kso,
            menejemen, totalByr, kdPj, status, kelas, kategori
          </SheetDescription>
        </SheetHeader>
